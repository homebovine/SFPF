...
for rep in range(rep_time):
    ...
    ###n-by-d matrix X
    A0 = tc.zeros(d, d)
    A0[:, 0:d] = tc.arange(0, d)
    M = abs(A0 - A0.transpose(1, 0))
    X_sig = tc.pow(0.5, M)
   # X0 = tc.normal(0, 1, (n, d))
    mean = tc.zeros(d)
    distrib = MultivariateNormal(loc=mean, covariance_matrix=X_sig)
    X0 = tc.zeros(n, d)
    for j in tc.arange(1, (n + 1)):
        X0[(j - 1), :] = distrib.sample()
    true_x[rep, :, :] = X0
    ### coefficient for W
    eta0 = tc.ones([dw])
    ### error
    err0 = tc.zeros(n, d)
    distrib2=StudentT(tc.tensor([3.0]))
    for j in tc.arange(1, (n + 1)):
        for t in tc.arange(1, (d + 1)):
            err0[(j - 1), (t - 1)] = distrib2.sample()
    ...
    true_mem = tc.hstack((tc.repeat_interleave(tc.Tensor([0]), d1), tc.repeat_interleave(tc.Tensor([1]), d2), tc.repeat_interleave(tc.Tensor([2]), d3)))
    adRI[rep, :] = metrics.adjusted_rand_score(true_mem.numpy(), opt_cluster_mem[rep, :])
    print(adRI[rep, :])
    with open('Setting2_t.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump(
...
###print results
with open('/root/zyq/Conc1/Setting2_t.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
    opt_cluster_mem, opt_cluster_size, opt_cluster_num, adRI, est_betad, est_beta,est_beta0, est_etad, est_eta, est_etaupp,est_etalow, true_beta1, true_beta2, true_beta3, true_z, true_y,  true_x, true_w, est_beta0upp, est_beta0low, est_std0, beta, alpha, lambda_tuning = pickle.load(f)
n = 40
d = 60
rep_time = 500
...
###The estimation results of Setting (2) in Table 3
